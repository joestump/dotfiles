...
#### A public artifact references only public forges and artifacts

Canonical is about **where the work goes**. It says nothing about **what you reference**, and the two pull in opposite directions: `{{ .giteaUrl }}` is a private, self-hosted instance behind auth that most readers cannot open at all. A Gitea reference in a public artifact is a dead end plus a small disclosure of internal infrastructure.

**So: anything that will be read outside the private Gitea MUST reference the GitHub copy** — `github.com/stump-wtf/<repo>` for a `stump.wtf` project, `github.com/{{ .githubUser }}/<repo>` for a personal one — or the project's public docs site. That covers, at minimum:
...
- **Commands a reader will actually run** — `git clone`, `go install`, install one-liners. This is the worst case, because a private host in a command **exits 0 for everyone inside the network**: it tests clean for us and fails only for the audience it was written for, with an error that looks like their broken toolchain. Note also that a GitHub *mirror* is a byte copy, so its `go.mod` still declares the private module path — `go install github.com/<org>/<repo>/cmd/x@main` fails with a module-path conflict. Clone the mirror and build from the checkout, or ship a public tap/release.
- **Citations inside a publicly distributed artifact** — issue references, SHA anchors, file paths in a skill or plugin README. A bare `#212` or a `docs/guides/…` path pointing into a private repo is unresolvable for an outside reader *and* for any agent they run. Describe the behaviour instead, or cite the public docs page; keep a SHA as plain provenance text rather than a link.

Gitea references are still correct — and still preferred — on the **internal** surfaces: a comment on a Gitea issue or PR, a Signal note to Joe, an Outline doc, a Cairn share, a commit message in a Gitea-canonical repo. The test is not "which host is canonical", it is **"can the reader reach this?"** If the answer is no, reference the mirror.
...
**Verify a replacement by content, not by status code.** Our docs sites are Docusaurus SPAs that return **200 for every path**, including nonsense ones, so a status check proves nothing; assert the page body contains what you expect. And run a command rather than resolving it — `go list -m` succeeding off a proxy cache is not the same as `go install` working for a stranger.
...
6. **Auto-merge is armed by the REVIEWER, never the author — and only once review is genuinely finished.** Nothing should sit green and approved waiting for someone to press a button. But arming it early is how work silently disappears, so the timing matters as much as the permission:

   - **Never as the author.** Not on a repo you own, not on an unprotected branch, not "just this once." Several of these repos require zero approvals (`stump.wtf/harness` `main` is status-checks-only, `joestump-agent/crush` `main` is unprotected), so arming it as the author lands agent-authored work with no human review at all — quietly downgrading the rule below to "whatever protection happens to require."
   - **As a reviewer, once review is genuinely finished** — not reflexively at the moment you click approve, while discussion is still open and the author may still push. Your approval *is* the gate that branch protection was protecting, so once it is on a finished PR there is nothing left to bypass, and the author is free of the button press. Approve only on green (see the review rules below); never arm auto-merge on a PR you have not approved, and never on your own.
   - **An armed auto-merge plus a late push silently drops the commit.** Seen 2026-09-12: a reviewer suggested a change, the author pushed it, and the merge took the **armed** SHA — the PR reported merged at a head that was never merged, then deleted the branch. `dismiss_stale_approvals` does not help, because the merge was armed before the push landed.
   - **After any push to a PR you believe merged, verify by CONTENT that your change reached `main`.** Not the `merged` flag, not the PR's head field, and **not** `git merge-base --is-ancestor` — these repos squash-merge, so a branch tip is never an ancestor even when the work landed perfectly. Read the file on `main` for the text you expect. Every other signal reads identically whether your commit landed or vanished.

   Both forges support it: GitHub `gh pr merge <n> --auto --squash`; Gitea 1.27+ `POST /api/v1/repos/{owner}/{repo}/pulls/{n}/merge` with `{"Do":"squash","merge_when_checks_succeed":true}`. The gate is always a real review — the sibling identity (`@{{ $agent }}` approves `@{{ $human }}`'s PRs and vice versa) reviews it, approves it, and arms the merge.
7. **Keep the merge style consistent with the repo.** Prefer a linear result: rebase or squash unless the repo says otherwise.
8. **Request review from the opposite identity.** Every PR gets a reviewer who is not its author, set as a *reviewer request* — not an assignee, which records ownership rather than review. When `@{{ $human }}` opens a PR, request review from `@{{ $agent }}`. When `@{{ $agent }}` opens a PR, request review from `@{{ $human }}`. Do it at PR creation rather than waiting for the scheduled sweep to notice. This is mandatory on repos owned by `{{ $human }}`, `stump.wtf`, or `stumpcloud`. On third-party repos, follow their review conventions instead. **Then ping the reviewer over Signal** — the two identities hold separate Signal numbers and can message each other directly (see the Cross-identity messaging docs in the dotfiles). A reviewer request alone can sit unnoticed until the scheduled sweep; a short Signal note — repo, PR number, one line on what changed — gets a human-authored PR reviewed in minutes instead of hours.

### Review requests are claimable work items — one reviewer, claim first

**A Signal ping addresses an identity, not a work item.** Every live session running as the pinged identity sees it, and nothing in Signal makes them mutually exclusive. Seen 2026-09-13 (stumpcloud#409): two `@{{ $agent }}` sessions took the same review request; one approved and merged nine seconds later while the other was still mid-review, burning a full independent test-suite run on a PR that had already landed. The tracker showed nothing wrong — one approval, one merge. The duplication is invisible everywhere except the wasted compute and the review whose verdict had nowhere to go.

So every review request is claimed before it is worked, and a duplicate exits in seconds, not minutes:

1. **Claim before any work.** If the request arrived as a Switchboard todo, `claim` that todo — the lease already makes it exclusive, and `claim_next` on a shared queue hands it to exactly one worker by construction. If it arrived as a bare Signal ping, look for the corresponding todo (`list_todos` for the repo/PR) and claim it; if it is already claimed by another session of the same identity, stop immediately and reply on the thread that a sibling owns it. No todo exists? Create the claim by posting a one-line Signal reply stating you are taking it **before** starting, then work it — and keep the cheap checks below regardless, because Signal carries no lease.
2. **Cheap state before expensive verification.** Re-read the PR's `state` / `merged` the moment you pick it up, and **again immediately before any long-running step** — a test suite, a CI reproduction, a build in the runner image. If it has landed, bail and report that; seconds of checking beat fourteen minutes of verifying a merged tree. The second session above would have been saved by this check alone.
3. **Approve and merge are separate motions.** Approve, then re-verify `mergeable` and that the head SHA is still the SHA you reviewed, then merge. Never approve-and-merge inside one motion to beat a sibling to it — a race to the merge button is the exact behaviour review exists to prevent, and an armed auto-merge plus a late push silently drops commits (see rule 6).
4. **A second independent review is deliberate or it does not happen.** Two reviewers disagreeing is signal — when it is wanted, it is asked for (by the first reviewer or by Joe) and recorded as a formal review on the PR. It is never an accident of fan-out that lands in Signal.

The durable fix is routing review requests through Switchboard so `claim_next` gives one todo to exactly one worker; until every request flows that way, the claim-then-verify protocol above is the rule every session runs, whatever channel the request arrived on.
...
4. **Lifecycle.** `claim` (lease, default 300s) → do the work → `complete` with a `result` recording what you did, or `fail` with a `result` recording why. `fail` retries while attempts remain, then dead-letters. On a queue you share with other workers, take work with **`claim_next`** rather than `claim`: it is the competing-consumers primitive, and it answers `{"empty": true}` when nothing is waiting.
5. **Heartbeat on a cadence while you work — not "if" the work runs long.** The lease is 300 seconds and real work routinely outruns it, so treat outrunning it as the normal case rather than the exception: `heartbeat` every couple of minutes, and again *before* you start anything slow — a build, a test suite, a clone, a long model call — rather than after it finishes. **A lapsed lease is not harmless.** The reaper returns the todo to `pending`, another worker claims it, and the same work is done twice; meanwhile your session keeps running and completes a todo somebody else has already redone. Nothing errors, and the duplicate is invisible from inside the session — which is why the rule is a cadence and not a judgement call.
6. **Never abandon a claim.** If you cannot finish it, `fail` it with a reason so it requeues rather than rotting under a stale lease.
