...
8. **Assign the opposite identity for review.** Every PR gets a reviewer who is not its author. When `@{{ $human }}` opens a PR, request review from `@{{ $agent }}`. When `@{{ $agent }}` opens a PR, request review from `@{{ $human }}`. This is mandatory on repos owned by `{{ $human }}`, `stump.wtf`, `stumpcloud`, or `{{ $agent }}` — self-merging without cross-review is not allowed. On third-party repos, follow their review conventions instead.
