...
# status — health panel: vault-agent, signal-cli daemon, dotfiles drift, disk.
# Uses gum style/join (color only shows on a TTY; plain text otherwise).
status() {
  emulate -L zsh
  _have gum || { print -u2 "status: needs gum"; return 1 }
  local host="${HOST%%.*}" va sd df disk pend
  ...
  local ok=150 bad=210 hl=117 warn=223 dim=244
  gum style --foreground "$hl" --bold "  StumpCloud · ${host}"
  gum join --vertical \
...
# dot — the action hub. One friendly front door to the common chores.
dot() {
  emulate -L zsh
  ...
  local choice
  choice=$(gum choose --header "StumpCloud · dotfiles" \
